...
type UpdateOptions struct {
	...
	PatchSpecToPod                 func(pod *v1.Pod, spec *UpdateSpec, state *workloadv1alpha1.InPlaceUpdateState) (*v1.Pod, map[string]*v1.ResourceRequirements, error)
	CheckPodUpdateCompleted        func(pod *v1.Pod) error
	CheckContainersUpdateCompleted func(pod *v1.Pod, state *workloadv1alpha1.InPlaceUpdateState) error
...
func (c *realControl) Refresh(pod *v1.Pod, opts *UpdateOptions) RefreshResult {
	...
	if gracePeriod, _ := workloadv1alpha1.GetInPlaceUpdateGrace(pod); gracePeriod != "" {
	...
	if stateStr, ok := workloadv1alpha1.GetInPlaceUpdateState(pod); ok {
		state := workloadv1alpha1.InPlaceUpdateState{}
	...
	newCondition := v1.PodCondition{
		Type:               workloadv1alpha1.InPlaceUpdateReady,
...
func (c *realControl) finishGracePeriod(pod *v1.Pod, opts *UpdateOptions) (time.Duration, error) {
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		spec := UpdateSpec{}
		updateSpecJSON, ok := workloadv1alpha1.GetInPlaceUpdateGrace(clone)
		if !ok {
			return nil
		}
		...
		updateState := workloadv1alpha1.InPlaceUpdateState{}
		updateStateJSON, ok := workloadv1alpha1.GetInPlaceUpdateState(clone)
		if !ok {
			return fmt.Errorf("pod has %s but %s not found", workloadv1alpha1.InPlaceUpdateGraceKey, workloadv1alpha1.InPlaceUpdateStateKey)
		}
		...
		if !c.revisionAdapter.EqualToRevisionHash("", clone, spec.Revision) {
			...
			workloadv1alpha1.RemoveInPlaceUpdateGrace(clone)
		} else {
			...
			clone, _, err = opts.PatchSpecToPod(clone, &spec, &updateState)
			if err != nil {
				return err
			}
			workloadv1alpha1.RemoveInPlaceUpdateGrace(clone)
...
func (c *realControl) updateNextBatch(pod *v1.Pod, opts *UpdateOptions) (bool, error) {
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		clone, _, err = opts.PatchSpecToPod(clone, &spec, &state)
		if err != nil {
			return err
		}
...
func (c *realControl) updatePodInPlace(pod *v1.Pod, spec *UpdateSpec, opts *UpdateOptions) (string, error) {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		clone.Annotations[workloadv1alpha1.InPlaceUpdateStateKey] = string(inPlaceUpdateStateJSON)
		delete(clone.Annotations, workloadv1alpha1.InPlaceUpdateStateKeyOld)

		if spec.GraceSeconds <= 0 {
			clone, _, err = opts.PatchSpecToPod(clone, spec, &inPlaceUpdateState)
			if err != nil {
				return err
			}
			workloadv1alpha1.RemoveInPlaceUpdateGrace(clone)
			...
			clone.Annotations[workloadv1alpha1.InPlaceUpdateGraceKey] = string(inPlaceUpdateSpecJSON)
		}
...
func InjectReadinessGate(pod *v1.Pod) {
	for _, r := range pod.Spec.ReadinessGates {
		if r.ConditionType == workloadv1alpha1.InPlaceUpdateReady {
			return
		}
	}
	pod.Spec.ReadinessGates = append(pod.Spec.ReadinessGates, v1.PodReadinessGate{ConditionType: workloadv1alpha1.InPlaceUpdateReady})
}
...
func GetCondition(pod *v1.Pod) *v1.PodCondition {
	return util.GetCondition(pod, workloadv1alpha1.InPlaceUpdateReady)
}
...
func doPreCheckBeforeNext(pod *v1.Pod, preCheck *workloadv1alpha1.InPlaceUpdatePreCheckBeforeNext) error {
	if preCheck == nil {
		return nil
	}
