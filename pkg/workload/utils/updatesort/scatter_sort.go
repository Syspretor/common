...
type scatterSort struct {
	strategy workloadv1alpha1.UpdateScatterStrategy
}

func NewScatterSorter(s workloadv1alpha1.UpdateScatterStrategy) Sorter {
...
func (ss *scatterSort) getScatterTerms(pods []*v1.Pod, indexes []int) []workloadv1alpha1.UpdateScatterTerm {
	if len(ss.strategy) == 1 {
		return ss.strategy
	}

	var termSlice []workloadv1alpha1.UpdateScatterTerm
	ruleCounter := map[string]int{}

	termID := func(term workloadv1alpha1.UpdateScatterTerm) string {
	...
	for _, term := range ss.strategy {
		for _, idx := range indexes {
			if val, ok := pods[idx].Labels[term.Key]; ok && val == term.Value {
				newTerm := workloadv1alpha1.UpdateScatterTerm{Key: term.Key, Value: val}
				id := termID(newTerm)
...
func (ss *scatterSort) scatterPodsByRule(term workloadv1alpha1.UpdateScatterTerm, pods []*v1.Pod, indexes []int) (ret []int) {
