...
const PVCOwnedByStsAnnotationKey = "workload.fluid.io/owned-by-asts"
...
type StatefulSetControlInterface interface {
	...
	UpdateStatefulSet(ctx context.Context, set *workloadv1alpha1.AdvancedStatefulSet, pods []*v1.Pod) error
	...
	ListRevisions(set *workloadv1alpha1.AdvancedStatefulSet) ([]*apps.ControllerRevision, error)
	// AdoptOrphanRevisions adopts any orphaned ControllerRevisions that match set's Selector. If all adoptions are
	// successful the returned error is nil.
	AdoptOrphanRevisions(set *workloadv1alpha1.AdvancedStatefulSet, revisions []*apps.ControllerRevision) error
}
...
func NewDefaultStatefulSetControl(
	...
	inplaceControl inplaceupdate2.Interface,
...
func (ssc *defaultStatefulSetControl) UpdateStatefulSet(ctx context.Context, set *workloadv1alpha1.AdvancedStatefulSet, pods []*v1.Pod) error {
	set = set.DeepCopy()
...
func (ssc *defaultStatefulSetControl) performUpdate(
	ctx context.Context, set *workloadv1alpha1.AdvancedStatefulSet, pods []*v1.Pod, revisions []*apps.ControllerRevision) (*apps.ControllerRevision, *apps.ControllerRevision, error) {
	var currentStatus *workloadv1alpha1.AdvancedStatefulSetStatus
	...
	klog.V(4).InfoS("AdvancedStatefulSet revisions", "statefulSet", klog.KObj(set),
...
func (ssc *defaultStatefulSetControl) ListRevisions(set *workloadv1alpha1.AdvancedStatefulSet) ([]*apps.ControllerRevision, error) {
...
// updateStatefulSet performs the update function for a AdvancedStatefulSet. This method creates, updates, and deletes Pods in
...
// Pods must be at Status.UpdateRevision. If the returned error is nil, the returned AdvancedStatefulSetStatus is valid and the
...
func (ssc *defaultStatefulSetControl) updateStatefulSet(
	...
	pods []*v1.Pod,
	revisions []*apps.ControllerRevision) (*workloadv1alpha1.AdvancedStatefulSetStatus, error) {
	selector, err := util.ValidatedLabelSelectorAsSelector(set.Spec.Selector)
	if err != nil {
	...
	if unhealthy > 0 {
		klog.V(4).InfoS("AdvancedStatefulSet has unhealthy Pods", "statefulSet", klog.KObj(set), "unhealthyReplicas", unhealthy, "pod", klog.KObj(firstUnhealthyPod))
	}

	// If the AdvancedStatefulSet is being deleted, don't do anything other than updating
	// status.
...
func (ssc *defaultStatefulSetControl) rollingUpdateStatefulsetPods(
...
) (*workloadv1alpha1.AdvancedStatefulSetStatus, error) {
	...
	minWaitTime := workloadv1alpha1.MaxMinReadySeconds * time.Second
	unavailablePods := sets.NewString()
	opts := &inplaceupdate2.UpdateOptions{}
	opts = inplaceupdate2.SetOptionsDefaults(opts)
	...
	for target := range replicas {
		...
		if !isHealthy(replicas[target]) {
			...
			klog.V(4).ErrorS(completedErr, "AdvancedStatefulSet found Pod in-place update not-ready",
	...
	klog.V(3).InfoS("Prepare to update pods indexes for AdvancedStatefulSet", "statefulSet", klog.KObj(set), "podIndexes", updateIndexes)
	...
	for _, target := range updateIndexes {
		...
		if len(unavailablePods) >= maxUnavailable && !unavailablePods.Has(replicas[target].Name) {
			klog.V(4).InfoS("AdvancedStatefulSet was waiting for unavailable Pods to update, blocked pod",
		...
		if !specifiedDeletedPods.Has(replicas[target].Name) && !isTerminating(replicas[target]) {
			...
			if !inplacing {
				klog.V(2).InfoS("AdvancedStatefulSet terminating Pod for update", "statefulSet", klog.KObj(set), "pod", klog.KObj(replicas[target]))
...
func (ssc *defaultStatefulSetControl) deletePod(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) (modified, actualDeleting bool, err error) {
	if set.Spec.Lifecycle != nil && lifecycle.IsPodHooked(set.Spec.Lifecycle.PreDelete, pod) {
		...
		if updated, _, err := ssc.lifecycleControl.UpdatePodLifecycle(pod, workloadv1alpha1.LifecycleStatePreparingDelete, markPodNotReady); err != nil {
			return false, false, err
		} else if updated {
			klog.V(3).InfoS("AdvancedStatefulSet scaling update pod lifecycle to PreparingDelete", "statefulSet", klog.KObj(set), "pod", klog.KObj(pod))
...
func (ssc *defaultStatefulSetControl) refreshPodState(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod, updateRevision string) (bool, time.Duration, error) {
	opts := &inplaceupdate2.UpdateOptions{}
	...
	case workloadv1alpha1.LifecycleStatePreparingUpdate:
		...
		if getPodRevision(pod) == updateRevision {
			if set.Spec.Lifecycle != nil && !lifecycle.IsPodAllHooked(set.Spec.Lifecycle.InPlaceUpdate, pod) {
				state = workloadv1alpha1.LifecycleStateUpdated
			} else {
				state = workloadv1alpha1.LifecycleStateNormal
			}
		}
	case workloadv1alpha1.LifecycleStateUpdating:
...
func (ssc *defaultStatefulSetControl) inPlaceUpdatePod(
	set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod,
	updateRevision *apps.ControllerRevision, revisions []*apps.ControllerRevision,
) (bool, error) {
	if set.Spec.UpdateStrategy.RollingUpdate == nil {
		return false, nil
	}
	if set.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy != workloadv1alpha1.InPlaceIfPossiblePodUpdateStrategyType &&
		set.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy != workloadv1alpha1.InPlaceOnlyPodUpdateStrategyType {
		return false, nil
	}
	...
	if ssc.inplaceControl.CanUpdateInPlace(oldRevision, updateRevision, opts) {
		...
		switch state {
		case "", workloadv1alpha1.LifecycleStatePreparingNormal, workloadv1alpha1.LifecycleStateNormal:
			var err error
			var updated bool
			if set.Spec.Lifecycle != nil && lifecycle.IsPodHooked(set.Spec.Lifecycle.InPlaceUpdate, pod) {
				markPodNotReady := set.Spec.Lifecycle.InPlaceUpdate.MarkPodNotReady
				if updated, _, err = ssc.lifecycleControl.UpdatePodLifecycle(pod, workloadv1alpha1.LifecycleStatePreparingUpdate, markPodNotReady); err == nil && updated {
					klog.V(3).InfoS("AdvancedStatefulSet updated pod lifecycle to PreparingUpdate", "statefulSet", klog.KObj(set), "pod", klog.KObj(pod))
				}
				return true, err
			}
		case workloadv1alpha1.LifecycleStateUpdated:
			var err error
			var updated bool
			var inPlaceUpdateHandler *workloadv1alpha1.LifecycleHook
			if set.Spec.Lifecycle != nil {
				inPlaceUpdateHandler = set.Spec.Lifecycle.InPlaceUpdate
			}
			if updated, _, err = ssc.lifecycleControl.UpdatePodLifecycleWithHandler(pod, workloadv1alpha1.LifecycleStatePreparingUpdate, inPlaceUpdateHandler); err == nil && updated {
				klog.V(3).InfoS("AdvancedStatefulSet updated pod lifecycle to PreparingUpdate", "statefulSet", klog.KObj(set), "pod", klog.KObj(pod))
			}
			return true, err
		case workloadv1alpha1.LifecycleStatePreparingUpdate:
		...
		if state != "" {
			opts.AdditionalFuncs = append(opts.AdditionalFuncs, lifecycle.SetPodLifecycle(workloadv1alpha1.LifecycleStateUpdating))
		}
	...
	if set.Spec.UpdateStrategy.RollingUpdate.PodUpdatePolicy == workloadv1alpha1.InPlaceOnlyPodUpdateStrategyType {
...
func updateStatus(status *workloadv1alpha1.AdvancedStatefulSetStatus, minReadySeconds int32, currentRevision, updateRevision *apps.ControllerRevision, podLists ...[]*v1.Pod) {
	status.Replicas = 0
...
func getStartOrdinal(set *workloadv1alpha1.AdvancedStatefulSet) int {
...
func (ssc *defaultStatefulSetControl) processCondemned(ctx context.Context, set *workloadv1alpha1.AdvancedStatefulSet, firstUnhealthyPod *v1.Pod, monotonic bool, condemned []*v1.Pod, i int) (bool, error) {
	logger := klog.FromContext(ctx)
	if isTerminating(condemned[i]) {
		// if we are in monotonic mode, block and wait for terminating pods to expire
		if monotonic {
			logger.V(4).Info("AdvancedStatefulSet is waiting for Pod to Terminate prior to scale down",
	...
	if !isRunningAndReady(condemned[i]) && monotonic && condemned[i] != firstUnhealthyPod {
		logger.V(4).Info("AdvancedStatefulSet is waiting for Pod to be Running and Ready prior to scale down",
	...
	if avail, waitTime := isRunningAndAvailable(condemned[i], getMinReadySeconds(set)); !avail && monotonic && condemned[i] != firstUnhealthyPod {
		logger.V(4).Info("AdvancedStatefulSet is waiting for Pod to be Available prior to scale down",
			"statefulSet", klog.KObj(set), "pod", klog.KObj(firstUnhealthyPod))
		if waitTime > 0 {
	...
	logger.V(2).Info("Pod of AdvancedStatefulSet is terminating for scale down",
		"statefulSet", klog.KObj(set), "pod", klog.KObj(condemned[i]))
...
// processReplica handles an individual replica within a AdvancedStatefulSet based on its current state.
...
func (ssc *defaultStatefulSetControl) processReplica(
	ctx context.Context,
	set *workloadv1alpha1.AdvancedStatefulSet,
	updateSet *workloadv1alpha1.AdvancedStatefulSet,
	monotonic bool,
	replicas []*v1.Pod,
	i int,
	status *workloadv1alpha1.AdvancedStatefulSetStatus,
	...
	if !isCreated(replicas[i]) {
		...
		state := workloadv1alpha1.LifecycleStatePreparingNormal
		if set.Spec.Lifecycle == nil ||
			set.Spec.Lifecycle.PreNormal == nil ||
		...
		if err := ssc.podControl.CreateStatefulPod(ctx, set, replicas[i]); err != nil {
			...
			condition := NewStatefulsetCondition(workloadv1alpha1.FailedCreatePod, v1.ConditionTrue, "", msg)
		...
		if monotonic {
			...
			logger.V(4).Info(
				"AdvancedStatefulSet pod is Creating, and break pods scale",
	...
	if isPending(replicas[i]) {
		logger.V(4).Info(
			"AdvancedStatefulSet is triggering PVC creation for pending Pod",
			"statefulSet", klog.KObj(set), "pod", klog.KObj(replicas[i]))
	...
	if isTerminating(replicas[i]) && monotonic {
		logger.V(4).Info("AdvancedStatefulSet is waiting for Pod to Terminate",
			"statefulSet", klog.KObj(set), "pod", klog.KObj(replicas[i]))
		return true, false, nil
	} else if isTerminating(replicas[i]) && decreaseAndCheckMaxUnavailable(scaleMaxUnavailable) {
		logger.V(4).Info(
			"AdvancedStatefulSet pod is Terminating, and break pods scale",
			"statefulSet", klog.KObj(set), "pod", klog.KObj(replicas[i]))
		return false, true, nil
	}
	...
	if err != nil {
		logger.Error(err, "AdvancedStatefulSet failed to update pod condition for inplace",
			"statefulSet", klog.KObj(set), "pod", klog.KObj(replicas[i]))
		return true, false, err
	} else if duration > 0 {
		durationStore.Push(getStatefulSetKey(set), duration)
	...
	if monotonic || scaleMaxUnavailable != nil {
		...
		if !isAvailable && monotonic {
			if waitTime > 0 {
				...
				durationStore.Push(getStatefulSetKey(set), waitTime)
				logger.V(4).Info(
					"AdvancedStatefulSet needs to wait for the pod to be Running and Available after being"+
				...
				logger.V(4).Info("AdvancedStatefulSet is waiting for Pod to be Available",
					"statefulSet", klog.KObj(set), "pod", klog.KObj(replicas[i]))
			}
			return true, false, nil
		} else if !isAvailable && decreaseAndCheckMaxUnavailable(scaleMaxUnavailable) {
			logger.V(4).Info(
				"AdvancedStatefulSet pod is unavailable, and break pods scale",
				"statefulSet", klog.KObj(set), "pod", klog.KObj(replicas[i]))
			if waitTime > 0 {
				// make sure we check later
				durationStore.Push(getStatefulSetKey(set), waitTime)
	...
	// Enforce the AdvancedStatefulSet invariants
	retentionMatch := true
	...
	if err := ssc.podControl.UpdateStatefulPod(updateSet, replica); err != nil {
		...
		condition := NewStatefulsetCondition(workloadv1alpha1.FailedUpdatePod, v1.ConditionTrue, "", msg)
		SetStatefulsetCondition(status, condition)
		return true, false, err
	}
...
func getScaleMaxUnavailable(set *workloadv1alpha1.AdvancedStatefulSet) (*int, error) {
