...
func (spc *StatefulPodControl) CreateStatefulPod(ctx context.Context, set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) error {
...
func (spc *StatefulPodControl) UpdateStatefulPod(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) error {
	...
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		// if the Pod does not conform to the AdvancedStatefulSet's storage requirements, update the Pod's PVC's,
		...
		// if the Pod's PVCs are not consistent with the AdvancedStatefulSet's PVC deletion policy, update the PVC
		// and dirty the pod.
...
func (spc *StatefulPodControl) DeleteStatefulPod(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) error {
...
func (spc *StatefulPodControl) ClaimsMatchRetentionPolicy(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) (bool, error) {
...
func (spc *StatefulPodControl) UpdatePodClaimForRetentionPolicy(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) error {
	ordinal := getOrdinal(pod)
	templates := set.Spec.VolumeClaimTemplates
	for i := range templates {
		claimName := getPersistentVolumeClaimName(set, &templates[i], ordinal)
...
func (spc *StatefulPodControl) PodClaimIsStale(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) (bool, error) {
	policy := getPersistentVolumeClaimRetentionPolicy(set)
	if policy.WhenScaled == workloadv1alpha1.RetainPersistentVolumeClaimRetentionPolicyType {
...
// recordPodEvent records an event for verb applied to a Pod in a AdvancedStatefulSet. If err is nil the generated event will
// have a reason of v1.EventTypeNormal. If err is not nil the generated event will have a reason of v1.EventTypeWarning.
func (spc *StatefulPodControl) recordPodEvent(verb string, set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod, err error) {
	if err == nil {
		reason := fmt.Sprintf("Successful%s", titleCase(verb))
		message := fmt.Sprintf("%s Pod %s in AdvancedStatefulSet %s successful",
			strings.ToLower(verb), pod.Name, set.Name)
		spc.recorder.Event(set, v1.EventTypeNormal, reason, message)
	} else {
		reason := fmt.Sprintf("Failed%s", titleCase(verb))
		message := fmt.Sprintf("%s Pod %s in AdvancedStatefulSet %s failed error: %s",
			strings.ToLower(verb), pod.Name, set.Name, err)
		spc.recorder.Event(set, v1.EventTypeWarning, reason, message)
	}
}

// recordClaimEvent records an event for verb applied to the PersistentVolumeClaim of a Pod in a AdvancedStatefulSet. If err is
...
func (spc *StatefulPodControl) recordClaimEvent(verb string, set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod, claim *v1.PersistentVolumeClaim, err error) {
	if err == nil {
		reason := fmt.Sprintf("Successful%s", titleCase(verb))
		message := fmt.Sprintf("%s Claim %s Pod %s in AdvancedStatefulSet %s success",
			strings.ToLower(verb), claim.Name, pod.Name, set.Name)
		spc.recorder.Event(set, v1.EventTypeNormal, reason, message)
	} else {
		reason := fmt.Sprintf("Failed%s", titleCase(verb))
		message := fmt.Sprintf("%s Claim %s for Pod %s in AdvancedStatefulSet %s failed error: %s",
			strings.ToLower(verb), claim.Name, pod.Name, set.Name, err)
		spc.recorder.Event(set, v1.EventTypeWarning, reason, message)
	}
}

// titleCase returns the string with the first rune converted to title case.
...
func (spc *StatefulPodControl) createMissingPersistentVolumeClaims(ctx context.Context, set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) error {
	if err := spc.createPersistentVolumeClaims(set, pod); err != nil {
		return err
	}
...
// set's Spec.
func (spc *StatefulPodControl) createPersistentVolumeClaims(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) error {
	var errs []error
	for _, claim := range getPersistentVolumeClaims(set, pod) {
		pvc, err := spc.objectMgr.GetClaim(claim.Namespace, claim.Name)
		switch {
