...
// statefulPodRegex is a regular expression that extracts the parent AdvancedStatefulSet and ordinal from the Name of a Pod
var statefulPodRegex = regexp.MustCompile("(.*)-([0-9]+)$")

// getParentNameAndOrdinal gets the name of pod's parent AdvancedStatefulSet and pod's ordinal as extracted from its Name. If
// the Pod was not created by a AdvancedStatefulSet, its parent is considered to be empty string, and its ordinal is considered
// to be -1.
...
// getParentName gets the name of pod's parent AdvancedStatefulSet. If pod has not parent, the empty string is returned.
 ...
 * managed by this AdvancedStatefulSet and is not listed in the reserveOrdinals.
 *
 * @return {boolean} True if the pod's ordinal is both within the allowed range and
 *                   not reserved; false otherwise.
 */
func podInOrdinalRange(pod *v1.Pod, set *workloadv1alpha1.AdvancedStatefulSet) bool {
...
func getPodName(set *workloadv1alpha1.AdvancedStatefulSet, ordinal int) string {
...
func getPersistentVolumeClaimName(set *workloadv1alpha1.AdvancedStatefulSet, claim *v1.PersistentVolumeClaim, ordinal int) string {
	// NOTE: This name format is used by the heuristics for zone spreading in ChooseZoneForVolume
	return fmt.Sprintf("%s-%s-%d", claim.Name, set.Name, ordinal)
}

// isMemberOf tests if pod is a member of set.
func isMemberOf(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) bool {
	return getParentName(pod) == set.Name
}

// identityMatches returns true if pod has a valid identity and network identity for a member of set.
func identityMatches(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) bool {
...
func storageMatches(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) bool {
	ordinal := getOrdinal(pod)
	if ordinal < 0 {
		return false
	}
...
// getPersistentVolumeClaimPolicy returns the PVC policy for a AdvancedStatefulSet, returning a retain policy if the set policy is nil.
func getPersistentVolumeClaimRetentionPolicy(set *workloadv1alpha1.AdvancedStatefulSet) workloadv1alpha1.StatefulSetPersistentVolumeClaimRetentionPolicy {
	policy := workloadv1alpha1.StatefulSetPersistentVolumeClaimRetentionPolicy{
		WhenDeleted: workloadv1alpha1.RetainPersistentVolumeClaimRetentionPolicyType,
		WhenScaled:  workloadv1alpha1.RetainPersistentVolumeClaimRetentionPolicyType,
	}
...
// PVC deletion policy for the AdvancedStatefulSet.
func claimOwnerMatchesSetAndPod(claim *v1.PersistentVolumeClaim, set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) bool {
	policy := getPersistentVolumeClaimRetentionPolicy(set)
	const retain = workloadv1alpha1.RetainPersistentVolumeClaimRetentionPolicyType
	const delete = workloadv1alpha1.DeletePersistentVolumeClaimRetentionPolicyType
	switch {
	default:
...
// the AdvancedStatefulSet. Returns true if the claim was changed and should be updated and false otherwise.
func updateClaimOwnerRefForSetAndPod(claim *v1.PersistentVolumeClaim, set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) bool {
	needsUpdate := false
	...
	updateMeta := func(tm *metav1.TypeMeta, kind string) {
		if tm.APIVersion == "" {
			if kind == "AdvancedStatefulSet" {
				tm.APIVersion = "workload.fluid.io/v1alpha1"
			} else {
				tm.APIVersion = "v1"
			}
		}
	...
	updateMeta(&setMeta, "AdvancedStatefulSet")
	policy := getPersistentVolumeClaimRetentionPolicy(set)
	const retain = workloadv1alpha1.RetainPersistentVolumeClaimRetentionPolicyType
	const delete = workloadv1alpha1.DeletePersistentVolumeClaimRetentionPolicyType
	switch {
...
func getPersistentVolumeClaims(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) map[string]v1.PersistentVolumeClaim {
	ordinal := getOrdinal(pod)
...
func updateStorage(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) {
...
func initIdentity(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) {
...
func updateIdentity(set *workloadv1alpha1.AdvancedStatefulSet, pod *v1.Pod) {
	ordinal := getOrdinal(pod)
	pod.Name = getPodName(set, ordinal)
	pod.Namespace = set.Namespace
	if pod.Labels == nil {
...
func isRunningAndAvailable(pod *v1.Pod, minReadySeconds int32) (bool, time.Duration) {
	...
	if state != "" && state != workloadv1alpha1.LifecycleStateNormal {
...
func allowsBurst(set *workloadv1alpha1.AdvancedStatefulSet) bool {
	return set.Spec.PodManagementPolicy == apps.ParallelPodManagement
}

// getMinReadySeconds returns the minReadySeconds set in the rollingUpdate, default is 0
func getMinReadySeconds(set *workloadv1alpha1.AdvancedStatefulSet) int32 {
...
func newStatefulSetPod(set *workloadv1alpha1.AdvancedStatefulSet, ordinal int) *v1.Pod {
	pod, _ := kubecontroller.GetPodFromTemplate(&set.Spec.Template, set, metav1.NewControllerRef(set, controllerKind))
	pod.Name = getPodName(set, ordinal)
...
// newVersionedStatefulSetPod creates a new Pod for a AdvancedStatefulSet. currentSet is the representation of the set at the
...
func newVersionedStatefulSetPod(currentSet, updateSet *workloadv1alpha1.AdvancedStatefulSet, currentRevision, updateRevision string,
...
func isCurrentRevisionNeeded(set *workloadv1alpha1.AdvancedStatefulSet, updateRevision string, ordinal int, replicas []*v1.Pod) bool {
...
// Match check if the given AdvancedStatefulSet's template matches the template stored in the given history.
func Match(ss *workloadv1alpha1.AdvancedStatefulSet, history *apps.ControllerRevision) (bool, error) {
...
// getPatch returns a strategic merge patch that can be applied to restore a AdvancedStatefulSet to a
// previous version. If the returned error is nil the patch is valid. The current state that we save is just the
// PodSpecTemplate. We can modify this later to encompass more state (or less) and remain compatible with previously
// recorded patches.
func getPatch(set *workloadv1alpha1.AdvancedStatefulSet) ([]byte, error) {
...
// ControllerRevision is valid. AdvancedStatefulSet revisions are stored as patches that re-apply the current state of set
// to a new AdvancedStatefulSet using a strategic merge patch to replace the saved state of the new AdvancedStatefulSet.
func newRevision(set *workloadv1alpha1.AdvancedStatefulSet, revision int64, collisionCount *int32) (*apps.ControllerRevision, error) {
...
// ApplyRevision returns a new AdvancedStatefulSet constructed by restoring the state in revision to set. If the returned error
// is nil, the returned AdvancedStatefulSet is valid.
func ApplyRevision(set *workloadv1alpha1.AdvancedStatefulSet, revision *apps.ControllerRevision) (*workloadv1alpha1.AdvancedStatefulSet, error) {
	clone := set.DeepCopy()
	patched, err := strategicpatch.StrategicMergePatch([]byte(runtime.EncodeOrDie(patchCodec, clone)), revision.Data.Raw, clone)
	if err != nil {
		return nil, err
	}
	restoredSet := &workloadv1alpha1.AdvancedStatefulSet{}
...
func inconsistentStatus(set *workloadv1alpha1.AdvancedStatefulSet, status *workloadv1alpha1.AdvancedStatefulSetStatus) bool {
...
// are set to 0.
func completeRollingUpdate(set *workloadv1alpha1.AdvancedStatefulSet, status *workloadv1alpha1.AdvancedStatefulSetStatus) {
	if set.Spec.UpdateStrategy.Type == apps.RollingUpdateStatefulSetStrategyType &&
...
// from the Pod. Pod's that have not been constructed by AdvancedStatefulSet's have an ordinal of -1, and are therefore pushed
...
// NewStatefulsetCondition creates a new advancedstatefulset condition.
...
func GetStatefulsetConditition(status workloadv1alpha1.AdvancedStatefulSetStatus, condType apps.StatefulSetConditionType) *apps.StatefulSetCondition {
...
// SetStatefulsetCondition updates the advancedstatefulset to include the provided condition. If the condition that
func SetStatefulsetCondition(status *workloadv1alpha1.AdvancedStatefulSetStatus, condition apps.StatefulSetCondition) {
...
/*
	apiVersion: workload.kruise.io/v1beta1
...
func getStatefulSetReplicasRange(set *workloadv1alpha1.AdvancedStatefulSet) (int, int, sets.Set[int]) {
